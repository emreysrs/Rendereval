...
function ResultsContent() {
  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 53px)', background: '#1F1F1E', display: 'flex', flexDirection: 'column' }}>
      {/* Breadcrumb */}
      <div style={{ padding: '10px 24px', borderBottom: '1px solid rgba(200, 200, 200, 0.2)', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px' }}>
        <span style={{ color: '#6b7280' }}>{projectType.toUpperCase()}</span>
        <span style={{ color: '#595F61' }}>›</span>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        {/* Left Sidebar */}
        <div style={{ width: '80px', borderRight: '1px solid #3F403F', overflowY: 'auto', padding: '8px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {frames.map((frame, i) => (
            <button key={frame.id} onClick={() => setSelectedFrame(i)} style={{ width: '100%', aspectRatio: '16/10', borderRadius: '6px', border: selectedFrame === i ? '2px solid #2b2b2b' : '1px solid #595F61', background: '#3F403F', cursor: 'pointer', position: 'relative', overflow: 'hidden', padding: 0 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '20px' }}>
          <div style={{ flex: 1, background: '#3F403F', borderRadius: '8px', border: '1px solid #595F61', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '300px' }}>
            <span style={{ color: '#6b7280', fontSize: '14px' }}>Frame {selectedFrame + 1} / {data.summary.shotName || 'Akeleti'}</span>
          </div>

          {/* Timeline */}
          <div style={{ marginTop: '16px' }}>
            <div style={{ width: '100%', height: '24px', background: '#3F403F', borderRadius: '4px', position: 'relative', overflow: 'hidden', display: 'flex' }}>
        ...
        {/* Right Sidebar */}
        <div style={{ width: '260px', borderLeft: '1px solid #3F403F', padding: '16px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {metricsArray.map((m) => (
            <div key={m.name} style={{ marginBottom: '4px' }}>
              ...
              <div style={{ width: '100%', height: '4px', background: '#3F403F', borderRadius: '2px', overflow: 'hidden' }}>
          ...
          {/* Score Card */}
          <div style={{ background: '#3F403F', border: '1px solid #595F61', borderRadius: '10px', padding: '20px', textAlign: 'center', marginTop: '8px' }}>
            <div style={{ fontSize: '36px', fontWeight: 700, color: '#2b2b2b', lineHeight: 1 }}>{data.overallScore}</div>
          ...
          <button onClick={() => router.push(`/report?type=${projectType}`)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: 'none', background: '#2b2b2b', color: '#fff', fontSize: '12px', fontWeight: 600, cursor: 'pointer', marginTop: '4px' }}>
            Continue to Dashboard
          </button>
        </div>
      </div>

      <div style={{ display: 'flex', justifyContent: 'center', padding: '12px', borderTop: '1px solid #3F403F', gap: '8px' }}>
